...
- ⚠️ 阈值是**按大档**给的，A1 词表很抠所以沿用 5%。调阈值要三个方向一起想：
  「更严 → 更多重试 → 更慢」，别只改数字。
- ✅ **2026-09-20 双真源已合并，且阈值改用范文标定值**（`A1 5% / A2 8.1% / B1 3.6% / B2+ 1.6%`）。
  前端 `offCap()`（`index.html:1416`）与后端 `VOCAB_THRESHOLD` 现在**同值**
  （此前前端 1/2/3/5%、后端 5/4/3/2%，**方向相反** —— B2+ 恰好反了）。
  改任一处**必须同时改另一处**，同 `MIN_USABLE_TEXT` 的教训。

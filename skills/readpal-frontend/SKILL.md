...
VOCAB_THRESHOLD= {"A1":0.05, "A2":0.04, "B1":0.03, "B2":0.02}  # 超纲率阈值
```
...
- ⚠️ 阈值是**按大档**给的，A1 词表很抠所以放宽到 5%。调阈值要三个方向一起想：
  「更严 → 更多重试 → 更慢」，别只改数字。
- 🔴 **改这两个常量时必须看前端 `offCap()`（`index.html:1412`）** —— 它**另写了一套**上限给界面显示：
  `A1 1% / A2 2% / B1 3% / B2+ 5%`，与后端实际判的 `5%/4%/3%/2%` **不一致**（**B2+ 正好反了**）。
  这是 **双真源**，同 `MIN_USABLE_TEXT` 的教训：**改一边必须改另一边**，否则界面承诺和机器判定对不上。
- 🔴 **这套阈值与 cap 本身尚未标定**（2026-09-18 认定）：
  ...
  - 阈值数字是**拍的，没有依据** —— 要用**教研认可的合格范文**标定后再定。方案见 `docs/local-notes/32`。
- ⚠️ **重试是空转**：`wfInputs` 在重试循环**外**算一次（`index.html:2662`），
  `MAX_VOCAB_RETRY` 次重试传的是**完全相同的入参** —— 约束没变化，必然仍失败，白花 ≈150 秒。
  实测 4 篇实际产出 × 4 档 = **16 个样本 15 个 `exceed`**。要真起效，必须把上一稿超纲词回灌给生成。

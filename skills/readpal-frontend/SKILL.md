...
⚠️ **首次上线实测（2026-09-22）：图 B 的 `nodeSemCheck` 判据偏严、误报率高。**
真实跑一遍报了 A2 档 5 段错位，逐段对照后**基本都是误报** —— 实际是低档把母稿相邻段
的内容**平移/压缩**了（删掉引入句、把第 2 段尾句挪到第 3 段开头），不是「讲成另一件事」。
⇒ **当前那句「可对相关档位重新生成」会误导教研去重跑本来合格的一档。**
待 Bryan 拍板把判据放宽成「事件序列整体一致（允许 ±1 段平移）」后，同步改这句文案。

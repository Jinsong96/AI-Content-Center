...
2. **`need_simplify` 必须始终显式传 `'true'` / `'false'`** —— 不能像以前那样"不勾整个省略该键"。
   （省略会让图 A 提示词里的 `{{#nodeStart.need_simplify#}}` 取不到值。）
3. **段数没有上限**：`LIC_MAX_SEG` 已删。`LIC_REF_MIN=6` / `LIC_REF_MAX=30` 只用于**导入阶段估算字数区间**，
   **不决定实际段数**（实际段数由图 A 按内容分出来，可能更少也可能更多）。
4. **`licSegBad()` 是"每段词数是否标红"的唯一判据**：只有 `prep.simplified === true` 时才判。
   保留原文时每段长度与档位规格无关 —— 一律标红 = 满屏红、真问题反而看不见。
5. **段数 ≠ 12 只提示不阻断**：黄条 + 一颗「仍然继续生成」（`onclick="licRunGen()"`），
   底部原本的「确认，生成 …」照旧可用。**任何段数都要能往下走。**

### 来自图 A 的三个信号（都要读，别只看 `warn`）

| 字段 | 含义 | 前端行为 |
|---|---|---|
| `ok` | **只有勾了精简**才会 false（段数≠12 或词数越界） | 红条 + 重跑出口 |
| `seg_note` | 段数偏离 12 的非阻断提示（如「按内容分成 19 段（推荐 12 段）」） | 黄条 + 继续按钮 |
| `fallback` | 勾了精简却没拿到精简稿 → 已自动回落为保留原文 | **必须显式提示**（否则老师以为拿到了精简稿） |

### 校验面板

校验分母用后端回传的 `vj.total_checks`（授权链路 **33** = 3 档 × 11；热点链路 44），**不要写死**。
母稿档不在校验结果里，界面上要写一句「母稿档 X 不参与规格校验（长度不由我们控制）」。

### 回归

```bash
cd frontend && python3 -m http.server 8899 &
node tools/probe_lic_flow.mjs --url=http://127.0.0.1:8899/index.html --port=9242   # 37 项断言
```

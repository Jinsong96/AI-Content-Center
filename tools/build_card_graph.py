...
🔴 **词数要「落进」每张卡给的区间 —— 低于下限和高过上限一样不合格。**
句数则是**上限**（只许少、不许多）：多写一句几乎必然把词数顶出去。
写少了就补一个次要细节回去，写多了就删 —— 两头都要往区间里收。
...
## 拆句（低级别最容易失分的地方）
- 🔴 **断句只看句号、问号、叹号（`.` `?` `!`）**。逗号、分号、冒号**都不算断句** ——
  原文里的 `olive oil, oily fish, whole grains, and lots of fruit and vegetables` 用逗号串成一串，
  代码数出来仍然是**一句**、20 多个词，直接判不合格。所以列举必须真的断成短句（把 `, and` 改成 `. `）。
- 单句词数是**硬上限**，不是建议：A1- ≤12、A2 ≤16、B1 ≤22（**B1 也一样**，不是只管低级别）。
  写完回头**逐句数字数**（只按 `. ? !` 数），超了就断成两句再交。
...
- 🔴 **主题词必须逐字出现在母稿正文里（硬性，选之前先在母稿里查一遍）**：B2+ 那一档就是母稿原文、
  一个字都不许改，所以**母稿正文里没有的词，选了就永远不合格**。
  实测踩过（2026-09-24）：选 healthy，而这篇母稿正文里根本没有 healthy ⇒ B2+ 直接判「缺少主题词」，
  而且这一条**回炉也修不掉**（B2+ 正文是代码硬取的原文）。选词前先在母稿正文里搜一遍每个候选词。
- ⚠️ **选词前再把它套进 A1- 的句子骨架试一遍（实测第二高频的失败项）** ——
  「___ is/are …」「People feel ___」「X makes us ___」「We eat ___」。套不进去的词就**别选它当主题词**。
  抽象心理词（depression / anxiety / stress / evolution / organism）在 A1- 里只能用 very sad / worry 表达，
  所以它们**不能当主题词**；换成**具体、能在 A1- 独立成句**的词（food / sugar / mood 这类）。
- 原文中的习语，低级别改用直白的说法。
...
- 🔴 **上面那份带 JSON 的范例只示范「格式和难度」，它的题材、事实、题目、解析一个字都不许出现在你的输出里。**
  实测踩过（2026-09-24）：模型把范例里关于 honesty 的那道题（"Why is honesty about money different from…"）
  连同范例正文的引文一起抄进了食品题材的输出 ⇒ 质检报「解析引用的英文在本级正文里找不到」⇒ 整轮白跑。
  每一道题的题干和解析都必须来自**本次给的这 10 张卡**。
...
PARSE_CODE = r"""function main({ text, title_in, master_cards, fix_levels }) {
  ...
  //
  // ⛔ 不能用「按分隔符切词 + nk 前缀匹配」：页面传的 fix_levels 是**人话句子**
  //   （形如「B2+（本轮只输出这几档）：`levels` 里只放列出的档位…」），切出来的第一个词
  //   带中文前缀，nk 的前缀匹配必然落空 ⇒ asked=[] ⇒ ASKED 退回四档。
  //   实测后果（2026-09-24）：回炉第 3–5 轮**连续整轮作废**（报「缺 A1-/A2/B1」），
  //   每轮白烧十几秒，而且那一档真正的残留问题永远修不掉。
  //   → 改成**全文扫描档位名**，与人话里档位出现在哪个位置无关。
  const asked = [];
  (String(fix_levels || '').match(/A1-?|A2|B1|B2\+/gi) || []).forEach(function (s) {
    const k = nk(s); if (k && asked.indexOf(k) < 0) asked.push(k);
  });

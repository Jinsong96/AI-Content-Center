...
def gen_node(nid, spec_note):
    ...
    head = ('你是英语分级阅读教学专家，专精 CEFR 分级改写。任务：把已获授权的英文母稿'
            ...
            '   - 每一段**只保留最核心的一个意思**；该段的举例、数字罗列、次要论证、铺陈与过渡句'
            '**一律删掉**。只换更简单的词、却不删内容，**一定超字数**。\n'
    ...
    t += ('\n\n【篇幅 · 逐段硬约束（本链路：段数由母稿决定，**不是固定 12 段**）】\n'
          ...
          '（举例、个案、解释性从句、铺陈）或把两句并成一句；'
          '**不许把两段合成一段，也不许补写母稿里没有的内容。**\n'
          '- 段数、每段词数、全文词数**三者必须同时成立** —— 只满足一个不算合格。\n')

    # ── 句数预算（2026-09-21 关键杠杆）────────────────────────────────
    # 教训链：光写「每段 11–14 词」模型数不准（三轮实测 24.8 → 18.7 词/段，始终超）。
    # 而 2026-09-21 在 GEN 上的六轮实测结论是：**模型是按「每段句数 × 句长」铺段的**。
    # ⇒ 直接把「每段恰好 2 句 × 每句 X–Y 词」写出来 —— 句数是模型能数的，
    #   2 句 × 句长区间恰好等于每段词数区间（A1 2×6–7=12–14、A2 2×10–11=20–22、
    #   B1 2×14–17=28–34、B2+ 2×21–24=42–48），所以这是**同一个约束的另一种写法**，不是放宽标准。
    SENT = {'nodeGenA1': '6–7', 'nodeGenA2': '10–11', 'nodeGenB1': '14–17', 'nodeGenB2p': '21–24'}
    SB = SENT.get(nid)
    if SB and per:
        t += ('\n\n【每段句数预算 —— **照这个写，篇幅就会自动命中**】\n'
              ...
              '- ⚠️ 本预算**覆盖上文任何「每段 1–2 句 / 2–3 句」的表述**：本链路一律每段 2 句。\n'
